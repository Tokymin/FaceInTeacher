 ...
 */
 ...
 public class RegisterTask extends AsyncTask<Void,Integer,Boolean> {
    ...
    protected Boolean doInBackground(Void... voids) {
        ...
        Boolean res=false;
        try {
            URL realUrl = new URL("http://geek-team.xin:8081/LAR/tRegister");
            // 打开和URL之间的连接
            ...
            // 发送请求参数
            out.print("tId=" + RegisterActivity.name + "&" + "tPassword=" + RegisterActivity.pwd);
            // flush输出流的缓冲
            out.flush();
    ...
    protected void onPostExecute(Boolean result) {
        if(result){
            Toast.makeText(RegisterActivity.registerActivity,"注册成功", Toast.LENGTH_LONG).show();
        }else {
            Toast.makeText(RegisterActivity.registerActivity,"注册失败",Toast.LENGTH_LONG).show();
        }
    }
}
